...
class AegearMainWindow(tk.Tk):
    ...
    def _setup_main_ui_layout(self):
        ...
        self.main_pane = tk.PanedWindow(self,
                                        ...
                                        bg='lightgrey',
                                        sashwidth=6)
        self.main_pane.pack(side=tk.TOP, fill=tk.BOTH,
                            expand=True, padx=5, pady=(5, 0))

        self.bottom_controls_frame = tk.Frame(self)
        self.bottom_controls_frame.pack(
            side=tk.BOTTOM, fill=tk.X, expand=False, padx=5, pady=5)

        self.left_toolbox_frame = tk.Frame(
            self.main_pane, bd=2, relief=tk.RAISED)
        ...
        calib_frame = tk.LabelFrame(
            self.left_toolbox_frame, text="Calibration")
        calib_frame.pack(side=tk.TOP, fill=tk.X,
                         expand=False, **toolbox_padding)

        self.calibration_button = tk.Button(
            calib_frame, text="Calibrate", command=self._calibrate, state=tk.DISABLED)
        self.calibration_button.pack(side=tk.TOP, fill=tk.X, pady=2, padx=5)

        track_control_frame = tk.LabelFrame(
            self.left_toolbox_frame, text="Tracking Control")
        track_control_frame.pack(
            side=tk.TOP, fill=tk.X, expand=False, **toolbox_padding)

        self.set_track_start_button = tk.Button(
            track_control_frame, text="Set Track Start", command=self._set_track_start)
        self.set_track_start_button.pack(
            side=tk.TOP, fill=tk.X, pady=2, padx=5)
        self.set_track_end_button = tk.Button(
            track_control_frame, text="Set Track End", command=self._set_track_end)
        self.set_track_end_button.pack(side=tk.TOP, fill=tk.X, pady=2, padx=5)

        self.run_tracking_button = tk.Button(
            track_control_frame, text="Run Tracking", command=self._run_tracking)
        self.run_tracking_button.pack(side=tk.TOP, fill=tk.X, pady=2, padx=5)

        self.reset_tracking_button = tk.Button(
            track_control_frame, text="Reset Tracking", command=self._reset_tracking)
        ...
        cleanup_frame.pack(side=tk.TOP, fill=tk.X,
                           expand=False, **toolbox_padding)
        ...
        self.highlight_outliers_button.pack(
            side=tk.TOP, fill=tk.X, pady=2, padx=5)
        ...
        self.outlier_threshold_scale.pack(
            side=tk.TOP, fill=tk.X, pady=2, padx=5)

        nav_outlier_frame = tk.Frame(cleanup_frame)
        nav_outlier_frame.pack(side=tk.TOP, fill=tk.X,
                               expand=False, pady=2, padx=5)

        self.prev_outlier_button = tk.Button(
            nav_outlier_frame, text="Previous Outlier", command=self._goto_previous_outlier)
        self.prev_outlier_button.pack(
            side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 2))
        self.next_outlier_button = tk.Button(
            nav_outlier_frame, text="Next Outlier", command=self._goto_next_outlier)
        self.next_outlier_button.pack(
        ...
        self.delete_outliers_button.pack(
            side=tk.TOP, fill=tk.X, pady=2, padx=5)
        ...
        self.delete_all_outliers_button.pack(
            side=tk.TOP, fill=tk.X, pady=2, padx=5)

        # --- Video Information section ---
        video_info_frame = tk.LabelFrame(
            self.left_toolbox_frame, text="Video Information")
        video_info_frame.pack(side=tk.TOP, fill=tk.X,
                              expand=False, **toolbox_padding)
        ...
        for label, key in info_fields:
            ...
            tk.Label(row, text=label+":", anchor=tk.W,
        ...
        track_params_frame = tk.LabelFrame(
            self.left_toolbox_frame, text="Parameters")
        track_params_frame.pack(side=tk.TOP, fill=tk.X,
                                expand=False, **toolbox_padding)

        self.tracking_threshold_scale = tk.Scale(
            track_params_frame, from_=0, to=100, orient=tk.HORIZONTAL, label="Tracking Thresh", command=self._tracking_threshold_changed)
        self.tracking_threshold_scale.set(
            int(self._tracker.tracking_threshold * 100.0))
        self.tracking_threshold_scale.pack(
            side=tk.TOP, fill=tk.X, pady=1, padx=5)

        self.detection_threshold_scale = tk.Scale(
            track_params_frame, from_=0, to=100, orient=tk.HORIZONTAL, label="Detection Thresh", command=self._detection_threshold_changed)
        ...
        self.tracking_frame_scale = tk.Scale(
            track_params_frame, from_=1, to=100, orient=tk.HORIZONTAL, label="Frame Skip", command=self._trajectory_frame_skip_changed)
        ...
        display_options_frame = tk.LabelFrame(
            self.left_toolbox_frame, text="Display")
        display_options_frame.pack(
            side=tk.TOP, fill=tk.X, expand=False, **toolbox_padding)

        self.draw_trajectory_checkbox = tk.Checkbutton(
            display_options_frame, text="Draw Trajectory", variable=self._draw_trajectory)
        self.draw_trajectory_checkbox.pack(
            side=tk.TOP, anchor=tk.W, pady=2, padx=5)
        ...
        self._tree_column_headers = {
            "frame": "FRAME", "centroid": "CENTROID", "confidence": "CONFIDENCE"}
        self._tree_col_widths = {"frame": 80,
                                 "centroid": 150, "confidence": 100}
        self._tree_font = ("Courier", 10)

        # Create a style for the Treeview
        style = ttk.Style(self)
        style.configure("Custom.Treeview", font=self._tree_font,
                        rowheight=int(self._tree_font[1] * 1.5))
        style.configure("Custom.Treeview.Heading", font=(
            self._tree_font[0], self._tree_font[1], 'bold'))
        ...
        self.tracking_tree.bind('<<TreeviewSelect>>',
                                self._treeview_item_selected)
        # Or handle deletion differently
        self.tracking_tree.bind('<Delete>', self._treeview_item_deleted)
        ...
        self.slider = tk.Scale(video_controls_frame, from_=0, to=self._num_frames,
                               orient=tk.HORIZONTAL, showvalue=0, command=self.slider_value_changed)
        self.slider.pack(side=tk.TOP, fill=tk.X,
                         expand=True, padx=5, pady=(0, 2))

        self.track_bar = TrackingBar(
            video_controls_frame, self._num_frames, height=10)
        self.track_bar.pack(side=tk.TOP, fill=tk.X,
        ...
        self.prev_frame_button = tk.Button(
            nav_time_frame, text=u"\u23EE", command=self.previous_frame)
        self.prev_frame_button.pack(side=tk.LEFT, padx=2)

        self.play_button = tk.Button(
            nav_time_frame, text=u"\u25B6", command=self._start_tracking)
        self.play_button.pack(side=tk.LEFT, padx=2)

        self.pause_button = tk.Button(
            nav_time_frame, text=u"\u23F8", command=self._pause_tracking)
        self.pause_button.pack(side=tk.LEFT, padx=2)

        self.next_frame_button = tk.Button(
            nav_time_frame, text=u"\u23ED", command=self.next_frame)
        self.next_frame_button.pack(side=tk.LEFT, padx=2)

        self.timeline_label = tk.Label(
            nav_time_frame, text="00:00:00", width=50)
        self.timeline_label.pack(side=tk.LEFT, padx=5)

        tk.Frame(nav_time_frame).pack(side=tk.RIGHT, expand=True)

        self.status_bar = tk.Label(
            self.bottom_controls_frame, text="Not Calibrated", fg="red", bd=1, relief=tk.SUNKEN, anchor=tk.W)
        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X, expand=False)

        self.distance_status_bar = tk.Label(
            self.bottom_controls_frame, text="Distance: 0.0 cm", fg="blue", bd=1, relief=tk.SUNKEN, anchor=tk.W)
    ...
    def _register_tracking_to_ui(self, frame_id: int, coordinates: tuple[int, int], confidence: float) -> None:
        ...
        """
        item_iid = str(frame_id)
        item_values = self._format_treeview_values(
            frame_id, coordinates, confidence)
        ...
        if self.tracking_tree.exists(item_iid):
            ...
            if all_current_iids_str:
                ...
                frame_ids.sort()
                insertion_index_in_sorted_list = bisect.bisect_left(
                    frame_ids, frame_id)
                self.tracking_tree.insert(
                    parent='',
    ...
    def _update_treeview_selection(self, target_frame_id):
        ...
        if self.tracking_tree.exists(target_iid):
            ...
            self.tracking_tree.see(target_iid)
        else:
            # TODO: same as above, find the right way to report errors.
            print(
                f"WARNING: Frame ID {target_frame_id} not found in Treeview for selection.")
    ...
    def _next_tracked_frame(self):
        ...
        insertion_point = bisect.bisect_right(
            self._sorted_tracked_frame_ids, current_frame_on_slider)
    ...
    def _create_menu(self):
        ...
        self.file_menu.add_command(
            label="Load Video", command=self._load_video)
        self.file_menu.add_command(
            label="Load Tracking", command=self._load_tracking)
        self.file_menu.add_command(
            label="Save Tracking", command=self._save_tracking)
        self.file_menu.add_command(
            label="Load Scene Reference", command=self._load_scene_reference)
    ...
    def _treeview_item_selected(self, event):
        ...
        )  # Returns a tuple of selected item IDs
        ...
        try:
            ...
            print(
                f"Error: Could not parse frame_id from Treeview item iid: {item_iid}")
    ...
    def _treeview_item_deleted(self, event):
        ...
        for item_iid in selected_items:
            try:
                ...
                print(
                    f"Error: Could not parse frame_id for deletion: {item_iid}")
    ...
    def _run_tracking(self):
        ...
        if self.track_bar.processing_start is None or self.track_bar.processing_end is None:
            messagebox.showerror(
                "Error", "Please set the processing start and end frames.")
            return
        ...
        self._current_frame = cv2.resize(
            self._current_frame, None, fx=0.1, fy=0.1)
        self._current_frame = cv2.GaussianBlur(self._current_frame, (3, 3), 0)
        self._current_frame = cv2.resize(
            self._current_frame, None, fx=10, fy=10)
    ...
    def _load_tracking(self):
        ...
        self.status_bar['text'] = "Tracking data loaded from {}".format(
            filename)
    ...
    def _do_calibration(self):
        ...
        """
        try:
            self._pixel_to_cm_ratio = self._scene_calibration.calibrate(
                self._screen_points)
            self._calibrated = True
            self._calibration_running = False
            self.status_bar['text'] = "Calibration complete: pixel to cm ratio is {}".format(
                self._pixel_to_cm_ratio)
            self.status_bar['fg'] = "green"
            ...
            self._calibrated = False
            self._calibration_running = False
            self.status_bar['text'] = "Calibration failed - internal error: {}".format(
                e)
            self.status_bar['fg'] = "red"
            self.calibration_button['text'] = "Calibrate"
    ...
    def _read_frame(self, frame_number):
        ...
        if self._clip is None:
            ...
            if logo_img is not None:
                ...
                blended = (roi * (1 - alpha_mask[..., np.newaxis]) +
                           overlay_rgb * alpha_mask[..., np.newaxis]).astype(np.uint8)
        ...
        frame = self._clip.get_frame(
            float(frame_number) / float(self._clip.fps))
        if self._calibrated:
    ...
    def _compute_travel_distance(self) -> tuple[float, float]:
        """Compute the total and current travel distance in cm using undistorted (calibrated) points.
        Returns (total_distance, current_distance)."""
        if not self._calibrated or not self._fish_tracking or not self._sorted_tracked_frame_ids:
            return 0.0, 0.0

        tracked_points = np.array([
            self._scene_calibration.rectify_point(self._fish_tracking[frame_id][0])
            for frame_id in self._sorted_tracked_frame_ids
        ])
    ...
    def _compute_trajectory_overlay(
        ...
        """
        window_seconds = self.video_canvas.trajectory_fade_seconds if hasattr(
            self, 'video_canvas') else 3.0
    ...
    def update_gui(self) -> None:
        ...
        calib_points = self._screen_points if self._calibration_running or (
            self._calibrated and not self._screen_points) else []
        track_point = self._get_current_track_point()

        # Compute trajectory overlay using helper
        trajectory_overlay = self._compute_trajectory_overlay(
            current_frame_id, fps)
        self.video_canvas.set_trajectory_overlay(trajectory_overlay)

        # Update the trajectory length and distance status bar.
        if self._calibrated and self._fish_tracking:
            ...
            # Just cleanup the travel distance status.
            self.distance_status_bar['text'] = ""
    ...
    def _highlight_outliers(self, show_message=True):
        ...
        if not self._outlier_frames and show_message:
            messagebox.showinfo("Highlight Outliers",
        ...
        if show_message:
            messagebox.showinfo(
                "Highlight Outliers", f"Highlighted {len(self._outlier_frames)} outlier(s) in yellow.")
    ...
    def _delete_current_outliers(self, show_message=True):
        ...
        if show_message:
            messagebox.showinfo(
                "Delete Outliers", f"Deleted {count} outlier(s) from tracking data.")
    ...
    def _update_video_info(self):
        ...
        if self._clip is not None:
            ...
            frames = int(self._clip.duration *
                         self._clip.fps) if self._clip.duration and self._clip.fps else '-'
            length = self._frame_to_time(frames, fps)
        else:
            filename = fps = resolution = length = frames = '-'

        self.video_info_labels["filename"].config(
